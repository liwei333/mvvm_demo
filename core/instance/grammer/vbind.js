import { getValue,mergeAttr, getEnvAttr } from "../../util/ObjectUtil.js";
...
function analysisExpression(vm, vnode, expressionList) {//解析表达式
    //获取当前环境的变量
    ...
    for (let i = 0; i < expressionList.length; i++) {
        ...
        if (site > -1) {
            let code = expressionList[i].substring(site + 1, expressionList[i].length);//obj.x < 2是这段
